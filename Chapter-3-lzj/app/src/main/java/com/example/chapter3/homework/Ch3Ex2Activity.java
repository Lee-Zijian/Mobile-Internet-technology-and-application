...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator fade_animator=ObjectAnimator.ofFloat(target,"alpha",1, 0.5f);
        fade_animator.setDuration(Integer.valueOf(durationSelector.getText().toString()));
        fade_animator.setRepeatCount(ObjectAnimator.INFINITE);
        fade_animator.setRepeatMode(ObjectAnimator.REVERSE);

        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleXAnimator,scaleYAnimator,fade_animator);
        animatorSet.start();
    }
}
